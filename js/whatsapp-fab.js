(function whatsappFab() {
  // Keep agent/admin dashboards clean; show floating button on public pages.
  ...
  function safeWhatsAppHref(value) {
    ...
    try {
      ...
      const allowed = WA_HOSTS.some(
        (h) => host === h.replace(/^www\./, "") || host.endsWith(".whatsapp.com")
      );
      if (!allowed) return "";
      return url.href;
    } catch {
  ...
  function mountFab(url) {
    if (!url || document.getElementById("whatsapp-fab-root")) return;
  ...
  function load(attempt) {
    ...
    api
      .getSiteSettings()
      .then((settings) => {
        const url = safeWhatsAppHref(settings?.whatsapp_channel_url || "");
        mountFab(url);
      })
      .catch(() => {});
  }
